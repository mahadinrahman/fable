...
const ManageBooks = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#090a0f] flex justify-center items-center text-white">
        <motion.span initial={{ opacity: 0.4 }} animate={{ opacity: 1 }} transition={{ repeat: Infinity, duration: 0.8, repeatType: "reverse" }}>
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto bg-[#02040a]/60 border border-indigo-950/40 rounded-xl backdrop-blur-xl p-6 shadow-2xl">
          ...
          <table className="w-full border-collapse text-left text-sm">
            ...
            >
              {books.length === 0 ? (
                ...
                books.map((book) => (
                  <motion.tr 
                    variants={staggeredRowVariants}
                    key={book._id} 
                    className="hover:bg-[#0d0e15]/30 transition-colors group"
                  >
                    <td className="py-4 px-4 flex items-center gap-4 max-w-sm">
                      <div className="w-12 h-16 rounded bg-slate-900 border border-indigo-950/50 shadow-md shrink-0 overflow-hidden">
                        <motion.img 
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center justify-center gap-4">
                        <motion.div whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.94 }}>
                          <Link 
                            href={`/books/${book._id}`} 
                            title="View Details"
                        ...
                        <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.94 }} onClick={() => openEditModal(book)} aria-label="Edit book">
                          <EditIcon />
                        </motion.button>

                        <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.94 }} onClick={() => handleDelete(book._id)} aria-label="Delete book">
                          <DeleteIcon />
                        </motion.button>
                      </div>
                    </td>
        ...
        {/* Dynamic Modal AnimatePresence Pipeline */}
        <AnimatePresence>
